...
# def parse_roadnet(f_path):
#     lines = open(f_path,'r').read().split('\n')[:-1]
#     n_inter = int(lines[0])
#     n_road = int(lines[n_inter+1])
#     records = lines[n_inter+2:n_inter+n_road*3:3]
#     return records


def build_inter_graph(f_path):
    graph_map = {} # ['inter_from]: [inter_to_1: {speed: xx, length: xx}, inter_to_2, ...]
    records = parse_roadnet(f_path)
    for s_record in records:
        record = s_record.split(' ')
        inter_id = int(record[0])
        if (inter_id not in graph_map.keys()):
            graph_map[inter_id] = [] # list of neighbours
        to_inter = {'id': int(record[1]), 'length': float(record[2]), 'sp_lim': float(record[3])}
        graph_map[inter_id].append(to_inter)
        ...
        while i < args.steps:
            if i % args.action_interval == 0:
                ...
                for key, val in observations.items():
                    observations_agent_id = int(key.split('_')[0]) #12988802954
                    observations_feature = key.split('_')[1]  #lane
